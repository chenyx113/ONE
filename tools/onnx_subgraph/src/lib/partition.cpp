      ...
      for (const auto &output_node : g.node())
      {
        ...
        if (find_flag == 1)
        {
          if (std::find(ad_node.outputNodeIndex.begin(), ad_node.outputNodeIndex.end(),
                        output_node_index) == ad_node.outputNodeIndex.end())
          {
            ad_node.outputNodeIndex.push_back(output_node_index);
          }
        }
        output_node_index++;
      }
...
void determine_subgraphs(const onnx::GraphProto &g, std::vector<onnx::GraphProto> &otherSubgraphs,
                         Device &d, int *visited, std::vector<GraphAdjacencyNode> &adjacency_list,
                         PartitionStrategy strategy)
{
  int _max_subgraph_size = d._max_subgraph_size;
  ...
  for (int i = 0; i < g.node_size(); i++)
  {
        ...
        (std::find(support_op.begin(), support_op.end(), g.node(i).op_type()) != support_op.end()))
    {
      ...
      DFS(g, subgraph, sugraph_node_index, visited, node, i, adjacency_list, support_op, prefer_op,
          depth, graph_size, _max_subgraph_size);
...
void determine_subgraphs_v2(const onnx::GraphProto &g,
                            ...
                            PartitionStrategy strategy)
{
  float _max_subgraph_size = d._max_subgraph_size;
  std::vector<std::string> support_op;
  std::vector<std::string> prefer_op;
  support_op = d.getNPUSupportOp();
  prefer_op = d.getNPUPreferOp();
  ...
  while (!end_flag)
  {
    ...
    if (temp_graph.node_size() != 0)
    {
      if ((std::find(support_op.begin(), support_op.end(), g.node(node_count).op_type()) !=
           support_op.end()) &&
          temp_graph.node_size() <= _max_subgraph_size)
      {
        *temp_graph.add_node() = g.node(node_count);
        temp_graph_size += node_size;
        if (temp_graph_size > _max_subgraph_size)
        {
          std::cout << "graph size exceed max size!" << temp_graph_size << " " << _max_subgraph_size
                    << std::endl;
        }
  ...
  for (int i = 0; i < g.node_size(); i++)
  {
    ...
    if (visited[i] == 0 && temp_graph_size2 < _max_subgraph_size)
    {
 ...
 * @exception None
 *
 * @return    A vector of `GraphAdjacencyNode` structures containing the nodes and their ranks.
 */
std::vector<GraphAdjacencyNode> calculate_node_rank(std::vector<int> &strongly_connected,
                                                      std::vector<onnx::GraphProto> &Subgraphs,
                                                      std::vector<onnx::GraphProto> &otherSubgraphs)
{
  onnx::GraphProto merged_graph;
  std::vector<GraphAdjacencyNode> node_rank_list;
 ...
 * @return    A vector of `GraphAdjacencyNode` structures containing the nodes and their ranks.
 */
std::vector<GraphAdjacencyNode> calculate_node_rank_v2(
 ...
 * @param     [out] node_rank_list A vector of `GraphAdjacencyNode` structures to store the nodes
 * and their ranks.
 *
 ...
 * the next rank.
 *
 * @param     [in] scc_node_rank A vector of `GraphAdjacencyNode` structures representing the
 ...
 */
std::vector<int> get_cut_rank_v2(std::vector<GraphAdjacencyNode> &scc_node_rank)
{
                      ...
                      std::vector<onnx::GraphProto> &otherSubgraphs, const onnx::GraphProto &g)
{
  ...
  for (auto &strongly_connected : strongly_connected_subgraphs)
    for (const auto scc_index : strongly_connected)
    {
      ...
      std::vector<GraphAdjacencyNode> scc_node_rank;
...
std::vector<int> cut_pair(std::vector<onnx::GraphProto> &Subgraphs,
                          ...
                          int subgraph_size)
{
  std::vector<GraphAdjacencyNode> pair_node_list =
                    ...
                    const std::unordered_map<std::string, NodeIOSize> &node_io_size)
{
  ...
  free(visited);
  std::vector<GraphAdjacencyNode>().swap(adjacency_list);
  int node_sum = 0;
